...
/**
 * Implementation of a {@link ServletOutputStream} that buffers all the output in
 * memory and delegates it to another stream if needed.
 * 
 * @author Dawid Weiss
...
final class DeferredOutputStream extends ServletOutputStream {
    private static final Logger log = Logger.getLogger(DeferredOutputStream.class.toString());
    ...
    /**
     * An exception saved when passing the buffered data to
     * {@link #passthroughStream}. 
     * 
    ...
    /**
     * Write the specified byte to the delegate stream or
     * buffer it.
     */
    ...
    /**
     * Write <code>b.length</code> bytes from the specified byte array to
     * the delegate stream or buffer it.
     * 
     ...
     * the specified offset, to the delegate stream or buffer it.
     * 
    ...
    /**
     * Sets a delegate stream. Any data buffered so far is written to the delegate
     * stream. Any future requests on this stream are directly delegated to <code>stream</code>,
     * without buffering. 
     */
    ...
    /**
     * Sets an {@link IOException} to be thrown on next call to any 
    ...
    private final void checkPendingExceptions() throws IOException {
        ...
        if (nextException != null) {
            log.log(Level.FINE, "Throwing pending exception.", nextException);
