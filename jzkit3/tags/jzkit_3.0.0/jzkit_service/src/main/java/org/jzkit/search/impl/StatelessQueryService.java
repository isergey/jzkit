...
public class StatelessQueryService implements ApplicationContextAware {
  ...
  private StatelessQueryService() {
    ...
    sessions_by_id = new LRUCache(100, 180000);  // 100 sessions, 3 min timeout
  }
  ...
  public StatelessSearchResultsPageDTO getResultsPageFor(String result_set_id,
    ...
    String query_string = model.toInternalQueryModel(ctx).toString();
    ...
    // After obtainint the result, if there is no cache, close down the session, it's never used again
    if ( cache_size == 0 ) {
